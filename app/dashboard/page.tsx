...
function Dashboard() {
  ...
  const [isSubmitProposalModalOpen, setIsSubmitProposalModalOpen] =
    useState(false);
  ...
  const selectedWallet = walletAddresses.find(
  ...
  const handleConnectWallet = async () => {
    if (isWalletConnected) {
      ...
      try {
        ...
        showToast(
          `Wallet connected with ${mockAddresses.length} addresses`,
          'success'
        );
      } catch (error) {
  ...
  const handlePurchaseQcap = async (amount: number) => {
    console.log(
      `Purchasing ${amount} QCAP for address ${selectedWalletAddress}`
    );
  ...
  const handleUnstakeQcap = async (amount: number) => {
    console.log(
      `Unstaking ${amount} QCAP from address ${selectedWalletAddress}`
    );
    ...
    showToast(
      `Unstaking ${amount} QCAP initiated. Available in 7 days.`,
      'success'
    );
    return new Promise<void>((resolve) => setTimeout(resolve, 1500));
  };
  ...
  const handleRegisterMuslimId = async (address: string, name: string) => {
    console.log(
      `Registering MuslimID for ${name} with wallet address ${address}`
    );
    // Implement actual registration logic here
    showToast(
      `Successfully registered ${name} as MuslimID with address ${address}`,
      'success'
    );
    return new Promise<void>((resolve) => setTimeout(resolve, 1500));
  };

  const handleTransferStocks = async (
    fromSC: string,
    toSC: string,
    amount: number
  ) => {
    ...
    showToast(
      `Stocks transferred successfully from ${fromSC} to ${toSC}`,
      'success'
    );
    return new Promise<void>((resolve) => setTimeout(resolve, 1500));
  };
        ...
        <div className='container flex h-16 items-center justify-between'>
          <div className='flex items-center gap-2'>
            <Image
              ...
              className='h-12 w-12'
            />
            <div className='text-primary font-bold text-2xl glow-text'>
              QCAP
            </div>
          ...
          <div className='hidden sm:flex items-center gap-4'>
            ...
            <Button
              ...
              className={
                isWalletConnected
                  ? 'w-full gap-2 glow-effect bg-green-600'
                  : 'w-full gap-2 border-primary/50'
              }
            >
              {isWalletConnected ? (
                <>
          ...
          <div className='sm:hidden border-t border-border/40 bg-background/95 backdrop-blur'>
            <div className='container py-4 space-y-2'>
              ...
              <Button
                ...
                className={
                  isWalletConnected
                    ? 'w-full gap-2 glow-effect'
                    : 'w-full gap-2 border-primary/50 hover:border-primary'
                }
              >
                {isWalletConnected ? (
                  <>
        ...
        {isWalletConnected ? (
          <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6'>
            <>
              ...
              >
                ...
                <span className='text-xs text-gray-400 mt-1'>
                  Register your wallet as MuslimID
                </span>
              </Button>
          ...
          <div className='bg-[#1a2035] rounded-lg p-6 mb-6 flex flex-col items-center justify-center'>
            ...
            <p className='text-gray-400 text-center mb-4'>
              Connect your wallet to access QCAP features including purchasing,
              MuslimID registration, stock transfers, and proposal submission.
            </p>
            <Button
